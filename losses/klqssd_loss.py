...
class KLQSSD_LOSS:
    ...
    def compute(self, y_true, y_pred):
        ...
        #
        quads_true = y_true[:, :, -28:-16]
        quads_pred = y_pred[:, :, -12:]
        class_true = y_true[:, :, :-28]
        class_pred = y_pred[:, :, :-12]
        #
        regression_loss = self.kl_divergence(quads_true, quads_pred)
        ...
        #
        pos_regression_loss = tf.reduce_sum(
            regression_loss * positives, axis=-1)
        pos_classification_loss = tf.reduce_sum(
            classification_loss * positives, axis=-1)
        #
        neg_classification_loss = classification_loss * negatives
        num_neg_classification_loss = tf.math.count_nonzero(
            neg_classification_loss, dtype=tf.int32)
        num_neg_classification_loss_keep = tf.minimum(
            tf.maximum(self.negative_boxes_ratio *
                       num_positives, self.min_negative_boxes),
        ...
        def f2():
            neg_classification_loss_1d = tf.reshape(
                neg_classification_loss, [-1])
            ...
            negatives_keep = tf.cast(tf.reshape(
                negatives_keep, [batch_size, num_boxes]), tf.float32)
            neg_class_loss = tf.reduce_sum(
                classification_loss * negatives_keep, axis=-1)
            return neg_class_loss

        neg_classification_loss = tf.cond(
            tf.equal(num_neg_classification_loss, tf.constant(0)), f1, f2)
        classification_loss = pos_classification_loss + neg_classification_loss

        total = (classification_loss + self.alpha * pos_regression_loss) / \
