...
from utils import ssd_utils, qssd_utils, one_hot_class_label, augmentation_utils, textboxes_utils, klqssd_utils
from time import time


class KLQSSD_DATA_GENERATOR(tf.keras.utils.Sequence):
    def __init__(
        self,
        samples,
        config,
    ...
    def __get_data(self, batch):
        ...
        for batch_idx, sample_idx in enumerate(batch):
            ...
            gt_qboxes = np.zeros((quads.shape[0], 12))
            ...
            y[batch_idx] = klqssd_utils.encode_qboxes(y[batch_idx])
            X.append(input_img)
