...
def generate_default_boxes_for_feature_map(
    ...
    cx = np.linspace(offset_x * grid_size, image_size -
                     (offset_x * grid_size), feature_map_size)
    cy = np.linspace(offset_y * grid_size, image_size -
                     (offset_y * grid_size), feature_map_size)
    cx_grid, cy_grid = np.meshgrid(cx, cy)
    cx_grid, cy_grid = np.expand_dims(
        cx_grid, axis=-1), np.expand_dims(cy_grid, axis=-1)
    cx_grid, cy_grid = np.tile(cx_grid, (1, 1, num_default_boxes)), np.tile(
        cy_grid, (1, 1, num_default_boxes))
    #
    default_boxes = np.zeros(
        (feature_map_size, feature_map_size, num_default_boxes, 4))
    ...
    #
    variances_tensor = np.array(variances)
    variances_tensor = np.expand_dims(variances_tensor, axis=0)
    variances_tensor = np.expand_dims(variances_tensor, axis=0)
    variances_tensor = np.expand_dims(variances_tensor, axis=0)
    variances_tensor = np.tile(
        variances, (default_boxes.shape[0], default_boxes.shape[1], default_boxes.shape[2], 1))
