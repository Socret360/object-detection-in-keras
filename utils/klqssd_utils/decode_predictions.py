...
def decode_predictions(
    ...
    df_boxes = y_pred[..., -16:-12]
    variances = y_pred[..., -12:]
    #
    xmin = y_pred[..., -28] * \
        tf.sqrt(variances[..., 0]) * df_boxes[..., 2] + \
        (df_boxes[..., 0] - (df_boxes[..., 2] / 2))
    ymin = y_pred[..., -27] * \
        tf.sqrt(variances[..., 1]) * df_boxes[..., 3] + \
        (df_boxes[..., 1] - (df_boxes[..., 3] / 2))
    xmax = y_pred[..., -26] * \
        tf.sqrt(variances[..., 2]) * df_boxes[..., 2] + \
        (df_boxes[..., 0] + (df_boxes[..., 2] / 2))
    ymax = y_pred[..., -25] * \
        tf.sqrt(variances[..., 3]) * df_boxes[..., 2] + \
        (df_boxes[..., 1] + (df_boxes[..., 3] / 2))
    x1 = y_pred[..., -24] * tf.sqrt(variances[..., 4]) * \
        df_boxes[..., 2] + (df_boxes[..., 0] - (df_boxes[..., 2]/2))
    y1 = y_pred[..., -23] * tf.sqrt(variances[..., 5]) * \
        df_boxes[..., 3] + (df_boxes[..., 1] - (df_boxes[..., 3]/2))
    x2 = y_pred[..., -22] * tf.sqrt(variances[..., 6]) * \
        df_boxes[..., 2] + (df_boxes[..., 0] + (df_boxes[..., 2]/2))
    y2 = y_pred[..., -21] * tf.sqrt(variances[..., 7]) * \
        df_boxes[..., 3] + (df_boxes[..., 1] - (df_boxes[..., 3]/2))
    x3 = y_pred[..., -20] * tf.sqrt(variances[..., 8]) * \
        df_boxes[..., 2] + (df_boxes[..., 0] + (df_boxes[..., 2]/2))
    y3 = y_pred[..., -19] * tf.sqrt(variances[..., 9]) * \
        df_boxes[..., 3] + (df_boxes[..., 1] + (df_boxes[..., 3]/2))
    x4 = y_pred[..., -18] * tf.sqrt(variances[..., 10]) * \
        df_boxes[..., 2] + (df_boxes[..., 0] - (df_boxes[..., 2]/2))
    y4 = y_pred[..., -17] * tf.sqrt(variances[..., 11]) * \
        df_boxes[..., 3] + (df_boxes[..., 1] + (df_boxes[..., 3]/2))
    ...
    output_tensor = tf.nest.map_structure(
        ...
        tf.map_fn(fn=lambda x: filter_predictions(x),
                  ...
                  #   infer_shape=True,
                  fn_output_signature=tf.TensorSpec(
                      (num_predictions, 14), dtype=tf.float32),
